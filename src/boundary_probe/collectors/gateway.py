...
from boundary_probe.collectors._runner import CommandResult, DefaultRunner, SubprocessRunner
...
def collect_gateway(runner: SubprocessRunner | None = None) -> GatewaySlice:
    """Discover the default IPv4 gateway via `route print -4`, then ping it 4 times."""
    r = runner or DefaultRunner()

    route_result = r.run(["route", "print", "-4"], timeout_s=5.0)
    ...
    ping_result = r.run(["ping", "-4", "-n", "4", "-w", "1000", gateway_ip], timeout_s=8.0)
    if ping_result.timed_out:
        return GatewaySlice(reachable=False, gateway_ip=gateway_ip, rtt_ms=None,
                            note=f"ping to {gateway_ip} timed out after 8s")
